...
/// `skill_lint`. Run the structural linter on a SKILL.md. Tier-0 by
/// default; advisory rules under `strict=true`. Mirrors the CLI's
/// `lyra lint [--strict]` 1:1 — same outcome JSON shape, same rule ids.
fn tool_lint(id: &str, args: &std::collections::HashMap<String, String>) -> String {
    let raw_md = match args.get("skill_md") {
        Some(v) => v,
        None => return tool_err(id, "missing skill_md"),
    };
    ...
    // `strict` is an optional boolean. Absent or anything other than the
    // literal `true` token defaults to false. We don't unquote here —
    // booleans in MCP arg dicts come through as raw `true`/`false`.
    let strict = matches!(args.get("strict").map(|s| s.trim()), Some("true"));
    let outcome = crate::linter::lint(&md, strict);
    tool_ok(id, &crate::linter::outcome_to_json(&outcome))
}
...
mod tests {
    ...
    fn tools_list_advertises_all_four_gates() {
        ...
        let resp = handle_message(req).expect("response");
        for name in ["skill_bind", "skill_verify", "skill_refine", "skill_compose", "skill_merge", "skill_lint"] {
